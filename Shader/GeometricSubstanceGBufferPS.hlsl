...
[earlydepthstencil]
#endif
PSGBufferOut main(VS_OUT pin, bool isFrontFace : SV_IsFrontFace) : SV_TARGET
{
    ...
    //GbufferÇ…èoóÕèÓïÒÇÇ‹Ç∆ÇﬂÇÈ
    GBufferData data;
    data.baseColor = materialInfo.basecolor;
    data.emissiveColor = materialInfo.emissiveFactor;
    data.wNormal = N;
    data.wPosition = pin.wPosition.xyz;
    data.roughness = materialInfo.perceptualRoughness;
    data.metalness = materialInfo.metallic;
    data.occlusionFactor = materialInfo.occlusionFactor;
    data.occlusionStrength = materialInfo.occlusionStrength;
    
    return EncodeGBuffer(data,sceneData.viewProjection);
}
